...
public class RoutingServlet {
    ...
    public RedirectView q1(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if (calculatedResult == null || resultUser == null || resultUser.isEmpty()) {
            redirectAttributes.addFlashAttribute("message", "Invalid input. Please enter both numbers and a result.");
            return new RedirectView("/q1", true);
        }

        try {
            if (Double.parseDouble(resultUser) == calculatedResult) {
                redirectAttributes.addFlashAttribute("message", "Correct! Moving to Q2.");
            ...
            redirectAttributes.addFlashAttribute("message", "Result must be a number.");
            return new RedirectView("/q1", true);
        }
    }
    ...
    public RedirectView q2(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        try {
            if (Double.parseDouble(resultUser) == calculatedResult) {
                redirectAttributes.addFlashAttribute("message", "Correct! Moving to Q3.");
    ...
    public RedirectView q3(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        try {
            if (Double.parseDouble(resultUser) == calculatedResult) {
                redirectAttributes.addFlashAttribute("message", "Correct! Well done!");
                return new RedirectView("/next", true); // You can customize /next
            } else {
                redirectAttributes.addFlashAttribute("message", "Wrong answer, try again.");
                return new RedirectView("/q3", true);
            }
    ...
    public String nextView() {
    	...
    	return "view-next"; // Make sure this file exists (e.g., view-next.jsp or view-next.html)
    }
